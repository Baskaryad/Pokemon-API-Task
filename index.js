const getTypeColor = type => {
  ...
  const getPokemonsType = async pokeApiResults => {
    ...
    return types = pokemons.map(fulfilled => fulfilled.types.map(info => DOMPurify.sanitize(info.type.name)))
  }
  ...
  const getPokemons = async () => {
    try {
        ...
        if (!response.ok) {
         throw new Error('No response frompokeapi')
        }
        ...
        return pokemons
         ...
         console.log('Algo deu erraco:', error)
     }
  }
...
const handleNextPokemonsRender = () => {
    const pokemonsObserver = new IntersectionObserver(async ([lastPokemon], observor) => {
        if(!lastPokemon.isIntersecting){
            return
        }

        observor.unobserve(lastPokemon.target)
